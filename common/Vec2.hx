...
class Vec2 {
    ...
    public function mul(a:Float) : Vec2 {
  ...
  public function isValid() : Bool {
    return !Math.isNaN(x) && Math.isFinite(x) && !Math.isNaN(y) && Math.isFinite(y);
  ...
  // TODO: equals not fully implemented
    if (this == obj) return true;
    if (obj == null) return false;
    if(Type.getClass(this) != Type.getClass(obj)) return false;
    // if (getClass() != obj.getClass()) return false;
    var other : Vec2 = cast obj;
    if(Std.int(this.x) != Std.int(other.x)) return false;
    if(Std.int(this.y) != Std.int(other.y)) return false;
